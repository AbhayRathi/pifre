...
export function PropertyHeader({ property }: PropertyHeaderProps) {
  const dataVariant = property.dataQuality === "real" ? "real" : property.dataQuality === "partial" ? "partial" : "fallback";
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-ivory-400 mt-1">
            {property.city}, {property.county && `${property.county} County, `}{property.state}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant={dataVariant}>
            {property.dataQuality === "real" ? "Real Data" : property.dataQuality === "partial" ? "Partial Data" : "Fallback Data"}
          </Badge>
          <Badge variant="default">
            {property.sourceRecords.length} sources
          </Badge>
        </div>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        <InfoItem label="Parcel" value={property.parcelNumber || "Pending"} />
        <InfoItem label="Lot Size" value={property.lotSizeSqFt ? `${property.lotSizeSqFt.toLocaleString()} sf` : "TBD"} />
        <InfoItem label="Building" value={property.buildingSizeSqFt ? `${property.buildingSizeSqFt.toLocaleString()} sf` : "N/A"} />
